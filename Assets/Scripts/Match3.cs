...
public class Match3 : MonoBehaviour
{
    ...
    List<Point> isConnected(Point p, bool main)
    {
        ...
        foreach (Point dir in directions)  //Checking if there is 2 or more same shapes in the directions
        {
            ...
            if (same > 1) //if there are more than 1 of the same shape in the directions we it is a match
                AddPoints(ref connected, line); //Add these points to the overarching connected list
        }

        for (int i = 0; i < 2; i++)// Chekcing if we are in the middle of two of the same shapes
        {
            List<Point> line = new List<Point>();

            int same = 0;
            Point[] check = { Point.add(p, directions[i]), Point.add(p, directions[i + 2]) };
            foreach (Point next in check)//Check both sides of the piece, iif they are the same value, add them to the list
            {
        ...
        for (int i = 0; i < 4; i++) //Check for a 2x2
        {
            ...
            foreach (Point pnt in check)//Check both sides of the piece, iif they are the same value, add them to the list
            {
